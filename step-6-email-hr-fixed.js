...
// Step name : email_hr
// Uses Brevo API (HTTP) — sends to any email address
// ============================================================
...
export default defineComponent({
  async run({ steps, $ }) {
    ...
    const recColor = data.ai_rekomendasi === "PROCEED"
      ? { bg: "#052e16", text: "#86efac", border: "#166534" }
      : data.ai_rekomendasi === "REVIEW"
      ? { bg: "#1c1917", text: "#fcd34d", border: "#92400e" }
...
.score-bar-bg { background: #334155; border-radius: 8px; height: 6px; overflow: hidden; margin: 16px 40px; }
...
<div class="wrap">
  ...
  <div class="footer">
    ...
    // Send via Brevo API — sends to ANY email address
    await axios.post(
      "https://api.brevo.com/v3/smtp/email",
      {
        sender     : { name: "SmartRecruit AI", email: process.env.BREVO_SENDER_EMAIL },
        to         : [{ email: process.env.HR_EMAIL, name: "HR Team" }],
